 ...
 */
public class SearchAdapter extends BaseRecyclerAdapter<SearchAdapter.MainViewHolder> {
    private Context context;
    private List<NoteItemBean> noteItemBeanList;

    public SearchAdapter(Context context, List<NoteItemBean> noteItemBeanList) {
    ...
    public MainViewHolder onCreateViewHolder(ViewGroup parent, int viewType, boolean isItem) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_search_note_layout, parent, false);
        return new MainViewHolder(view);
    }
    ...
    public void onBindViewHolder(MainViewHolder holder, int position, boolean isItem) {
        ...
        final NoteItemBean noteItemBean = noteItemBeanList.get(position);
        //设置备忘录每条时间
        holder.tv_note_time.setText(RelativeDateFormat.getTimeStringAutoShort(noteItemBean.getUp_data(),true));
        //设置每条内容
        holder.tv_noteContent.setText(noteItemBean.getContent());
        //闹钟图片显示与否
        if (noteItemBean.getIs_warn() == 0) {
            ...
            if (noteItemBean.getWarn_data().getTime() - new Date().getTime() < 0) {
                ...
                holder.img_clock.setImageResource(R.drawable.note_clock_waker);
            }
        }
        //点击事件，跳转详情页
